...
const subjectTypes = ['国語', '数学', '英語', '理科', '社会', '小論文'];
...
const highSchoolNames = Array.from(
  new Set(
    flowerNames.flatMap((flowerName) => {
      const result: string[] = [];
      for (let i = 0; i < 3; i++) {
        result.push(
          flowerName +
...
const main = async () => {
  ...
  for (const highSchoolName of highSchoolNames) {
    ...
    for (let i = 0; i < methodNum; i++) {
      ...
      const documents = new Set(excludeRandomFromArray(documentTypes));
      if (methods[i] === '一般') {
        documents.delete('推薦状');
      } else {
        documents.add('推薦状');
        documents.add('志望動機書');
      }
      ...
      const subjects =
        methods[i] === '一般' ? excludeRandomFromArray(subjectTypes) : ['小論文'];
      ...
      await prisma.application.create({
        data: {
          ...
          method: methods[i],
          documents: Array.from(documents).join(','),
